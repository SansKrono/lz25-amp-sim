...
TEST_CASE ("Boot performance")
{
    ...
    (Catch::Benchmark::Chronometer meter)
    {
        ...
        std::vector<Catch::Benchmark::storage_for<LZ25AudioProcessor>> storage (size_t (meter.runs()));
    ...
    {
        auto gui = juce::ScopedJuceInitialiser_GUI {};
        std::vector<Catch::Benchmark::destructable_object<LZ25AudioProcessor>> storage (size_t (meter.runs()));
        for (auto& s : storage)
            s.construct();
